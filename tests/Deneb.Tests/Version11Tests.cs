...
public sealed class Version11Tests : IDisposable
{
    ...
    public void MigrationKeepsOriginalBackupAndRejectsFutureVersions()
    {
        ...
        using (var store = new StateStore(root)) { var state = store.Load(); Assert.Equal(2, state.Version); Assert.False(state.GloballyPaused); Assert.Equal("Тест", state.Jobs[0].Name); store.Save(state); }
        Assert.Equal(original, File.ReadAllText(path + ".v1.bak"));
        File.WriteAllText(path, "{\"Version\":99}");
        using var future = new StateStore(root); Assert.Throws<InvalidDataException>(() => future.Load()); Assert.Equal("{\"Version\":99}", File.ReadAllText(path));
    }
    ...
    public async Task SegmentRetryDoesNotHideOtherTransfersAndPauseCancelsWait()
    {
        ...
        Assert.Equal("Передача", s.Phase); Assert.Equal(1, s.Segments[0].Retry); Assert.True(s.Segments[0].RetryIn > TimeSpan.Zero);
    ...
    public async Task CrashStateRespectsGlobalPause()
    {
        ...
        Assert.Equal(DownloadState.Queued, e.Snapshots()[0].State); Assert.Equal("Общая пауза", e.Snapshots()[0].Phase);
    }
    [Fact]
