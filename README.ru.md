...
## Deneb 2.2: CLI и поиск (подготовка релиза)

```sh
deneb add 'https://example.org/file.zip' --destination ./downloads
deneb add --stdin < links.txt
deneb list --search фильм --filter active
deneb list --json
deneb show ID_ЗАДАЧИ
deneb pause ID_ЗАДАЧИ
deneb resume ID_ЗАДАЧИ
deneb remove ID_ЗАДАЧИ
```

`add` запускает отсутствующий фон; просмотр и действия с задачами — нет. ID — полный UUID или уникальный шестнадцатеричный префикс от 8 символов. Без ID `pause`/`resume` управляют общей паузой. Удаление сохраняет файлы и части; для удаления частей нужны оба флага `--delete-partial --yes`, итоговые файлы не удаляются никогда. Для приватных ссылок используйте stdin, чтобы не оставлять URL в истории оболочки. При неизвестном результате сначала проверьте очередь, не повторяйте команду вслепую.
...
Перед обновлением выполните `deneb stop` старым бинарником. В 2.2 используется IPC v3, хранилище остаётся v5. Ссылки на опубликованные сборки ниже пока ведут на 2.1; публикация 2.2 — после приёмки.

## Место и общий лимит скорости
...
Перед обновлением остановите старый фон. Хранилище v5 сохраняет очереди v1–v4 с отдельной резервной копией; имена и ID частей v4 не меняются, лимит по умолчанию отсутствует. IPC v3 не подключается к старому фону. Не открывайте очередь v5 в 2.0 и более ранних версиях.
