...
Состояние обновляется до v3, перед миграцией сохраняется отдельная копия исходной версии. Неизвестные исторические ошибки показываются с пометкой «Сообщение предыдущей версии»; имена файлов и URL не переводятся.
