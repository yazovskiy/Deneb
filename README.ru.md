...
`add` запускает отсутствующий фон; просмотр и действия с задачами — нет. ID — полный UUID или уникальный шестнадцатеричный префикс от 8 символов. Без ID `pause`/`resume` управляют общей паузой. Удаление сохраняет файлы и части; для удаления частей нужны оба флага `--delete-partial --yes`, готовые файлы можно отдельно отправить в корзину macOS через --trash --yes. Для приватных ссылок используйте stdin, чтобы не оставлять URL в истории оболочки. При неизвестном результате сначала проверьте очередь, не повторяйте команду вслепую.
...
Delete предлагает «Убрать из списка» (сохранить всё), «Удалить незавершённые данные» (пропустить готовые) и «В корзину и убрать из списка» (только готовые файлы, macOS). Действия с данными требуют дополнительного подтверждения. CLI: `deneb remove ID --trash --yes`. На Windows/Linux корзина недоступна; безвозвратной замены этому действию нет. F9 всегда сохраняет файлы.
