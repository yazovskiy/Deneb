    ...
    public static CliOptions Parse(string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            if (!literal && arg.StartsWith('-'))
            {
                ...
                else if (arg is "--json" or "--help" or "--version" or "--stdin" or "--delete-partial" or "--yes")
        ...
        if (command != null && command is not ("start" or "status" or "stop" or "pause" or "resume" or "add" or "list" or "show" or "remove")) throw new CliInputException();
        ...
        if (flags.Any(f => f is "--delete-partial" or "--yes") && command != "remove") throw new CliInputException();
        if (flags.Contains("--delete-partial") != flags.Contains("--yes")) throw new CliInputException("CliDeleteConfirm");
        ...
        try
        {
            return new() { Command = command, StateDirectory = options.TryGetValue("--state-dir", out var state) ? Path.GetFullPath(state) : null,
                Json = flags.Contains("--json"), Help = flags.Contains("--help"), Version = flags.Contains("--version"), Stdin = flags.Contains("--stdin"),
